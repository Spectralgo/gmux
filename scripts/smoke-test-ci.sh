...
SOCKET_PATH="/tmp/gmux-debug.sock"
STABILITY_WAIT=15

echo "=== Smoke Test ==="

# --- Find the built app ---
APP=$(find ~/Library/Developer/Xcode/DerivedData -path "*/Build/Products/Debug/Gmux DEV.app" -print -quit 2>/dev/null || true)
if [ -z "$APP" ]; then
  echo "ERROR: Built app not found in DerivedData"
  exit 1
fi
echo "App: $APP"
BINARY="$APP/Contents/MacOS/Gmux DEV"
if [ ! -x "$BINARY" ]; then
...
rm -f "$SOCKET_PATH"
pkill -x "Gmux DEV" 2>/dev/null || true
sleep 1

# --- Launch the app directly (not via `open`, which can silently fail on CI) ---
echo "Launching app..."
CMUX_SOCKET_MODE=allowAll CMUX_UI_TEST_MODE=1 "$BINARY" > /tmp/gmux-smoke-stdout.log 2>&1 &
APP_PID=$!
...
if ! kill -0 "$APP_PID" 2>/dev/null; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/gmux-smoke-stdout.log 2>/dev/null | tail -50 || true
  echo "--- debug log ---"
  tail -50 /tmp/gmux-debug.log 2>/dev/null || true
  echo "--- crash reports ---"
...
if [ "$SOCKET_READY" != "true" ]; then
  ...
  echo "--- stdout/stderr ---"
  cat /tmp/gmux-smoke-stdout.log 2>/dev/null | tail -30 || true
  echo "--- debug log ---"
  tail -30 /tmp/gmux-debug.log 2>/dev/null || true
  ls -la /tmp/gmux-debug* 2>/dev/null || true
  pgrep -la "Gmux" || echo "No Gmux processes found"
  exit 1
fi
