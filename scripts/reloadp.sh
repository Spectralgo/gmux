...
sleep 0.2
APP_PATH="$(
  find "$HOME/Library/Developer/Xcode/DerivedData" -path "*/Build/Products/Release/Gmux.app" -print0 \
...
if [[ -z "${APP_PATH}" ]]; then
  echo "Gmux.app not found in DerivedData" >&2
  exit 1
fi
...
APP_PROCESS_PATH="${APP_PATH}/Contents/MacOS/Gmux"
ATTEMPT=0
MAX_ATTEMPTS=20
