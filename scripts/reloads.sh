...
APP_NAME="Gmux STAGING"
BUNDLE_ID="com.gmuxterm.app.staging"
BASE_APP_NAME="Gmux"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
...
Release build with isolated "Gmux STAGING" identity. Runs side-by-side with
the production Gmux app.
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="Gmux STAGING ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.gmuxterm.app.staging.${TAG_ID}"
  fi
  if [[ "$DERIVED_SET" -eq 0 ]]; then
    DERIVED_DATA="/tmp/gmux-staging-${TAG_SLUG}"
  fi
fi
...
if [[ -f "$INFO_PLIST" ]]; then
  ...
  CMUX_SOCKET="/tmp/gmux-${STAGING_SLUG}.sock"
