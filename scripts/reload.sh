...
APP_NAME="Gmux DEV"
BUNDLE_ID="com.gmuxterm.app.debug"
BASE_APP_NAME="Gmux DEV"
DERIVED_DATA=""
NAME_SET=0
BUNDLE_SET=0
DERIVED_SET=0
...
CMUX_DEBUG_LOG=""
CLI_PATH=""
LAST_SOCKET_PATH_DIR="$HOME/Library/Application Support/gmux"
...
write_dev_cli_shim() {
  ...
  cat > "$target" <<EOF
#!/usr/bin/env bash
# gmux dev shim (managed by scripts/reload.sh)
set -euo pipefail

CLI_PATH_FILE="/tmp/gmux-last-cli-path"
...
echo "error: no reload-selected dev gmux CLI found. Run ./scripts/reload.sh --tag <name> first." >&2
exit 1
EOF
  chmod +x "$target"
}
...
select_cmux_shim_target() {
  local app_cli_dir="/Applications/Gmux.app/Contents/Resources/bin"
  local marker="gmux dev shim (managed by scripts/reload.sh)"
  local target=""
  local path_entry=""
  local candidate=""
...
write_last_socket_path() {
  ...
  echo "$socket_path" > /tmp/gmux-last-socket-path || true
}
...
tagged_derived_data_path() {
  local slug="$1"
  echo "$HOME/Library/Developer/Xcode/DerivedData/gmux-${slug}"
}
...
print_tag_cleanup_reminder() {
  ...
  while IFS= read -r -d '' path; do
    if [[ "$path" == /tmp/gmux-* ]]; then
      tag="${path#/tmp/gmux-}"
    elif [[ "$path" == "$HOME/Library/Developer/Xcode/DerivedData/gmux-"* ]]; then
      tag="${path#$HOME/Library/Developer/Xcode/DerivedData/gmux-}"
    else
      continue
    fi
    ...
    find /tmp -maxdepth 1 -name 'gmux-*' -print0 2>/dev/null
    find "$HOME/Library/Developer/Xcode/DerivedData" -maxdepth 1 -type d -name 'gmux-*' -print0 2>/dev/null
  )
  ...
  if [[ "${#stale_tags[@]}" -eq 0 ]]; then
    ...
    for tag in "${stale_tags[@]}"; do
      echo "  pkill -f \"Gmux DEV ${tag}.app/Contents/MacOS/Gmux DEV\""
      echo "  rm -rf \"$(tagged_derived_data_path "$tag")\" \"/tmp/gmux-${tag}\" \"/tmp/gmux-debug-${tag}.sock\""
      echo "  rm -f \"/tmp/gmux-debug-${tag}.log\""
      echo "  rm -f \"$HOME/Library/Application Support/gmux/cmuxd-dev-${tag}.sock\""
    done
  fi
  echo "After you verify current tag, cleanup command:"
  echo "  pkill -f \"Gmux DEV ${current_slug}.app/Contents/MacOS/Gmux DEV\""
  echo "  rm -rf \"$(tagged_derived_data_path "$current_slug")\" \"/tmp/gmux-${current_slug}\" \"/tmp/gmux-debug-${current_slug}.sock\""
  echo "  rm -f \"/tmp/gmux-debug-${current_slug}.log\""
  echo "  rm -f \"$HOME/Library/Application Support/gmux/cmuxd-dev-${current_slug}.sock\""
}
...
if [[ -n "$TAG" ]]; then
  ...
  if [[ "$NAME_SET" -eq 0 ]]; then
    APP_NAME="Gmux DEV ${TAG}"
  fi
  if [[ "$BUNDLE_SET" -eq 0 ]]; then
    BUNDLE_ID="com.gmuxterm.app.debug.${TAG_ID}"
  fi
...
XCODE_LOG="/tmp/gmux-xcodebuild-${TAG_SLUG}.log"
set +e
...
if [[ -n "${TAG_SLUG:-}" ]]; then
  TMP_COMPAT_DERIVED_LINK="/tmp/gmux-${TAG_SLUG}"
...
if [[ -n "$TAG" && "$APP_NAME" != "$SEARCH_APP_NAME" ]]; then
  ...
  if [[ -f "$INFO_PLIST" ]]; then
    ...
    if [[ -n "${TAG_SLUG:-}" ]]; then
      APP_SUPPORT_DIR="$HOME/Library/Application Support/gmux"
      CMUXD_SOCKET="${APP_SUPPORT_DIR}/cmuxd-dev-${TAG_SLUG}.sock"
      CMUX_SOCKET="/tmp/gmux-debug-${TAG_SLUG}.sock"
      CMUX_DEBUG_LOG="/tmp/gmux-debug-${TAG_SLUG}.log"
      write_last_socket_path "$CMUX_SOCKET"
      echo "$CMUX_DEBUG_LOG" > /tmp/gmux-last-debug-log-path || true
...
if [[ "$LAUNCH" -eq 1 ]]; then
  ...
  if [[ -n "${TAG_SLUG:-}" && -n "${CMUX_SOCKET:-}" ]]; then
    ...
    echo "/tmp/gmux-debug.sock" > /tmp/gmux-last-socket-path || true
    echo "/tmp/gmux-debug.log" > /tmp/gmux-last-debug-log-path || true
...
if [[ -x "${CLI_PATH:-}" ]]; then
  ...
  echo "CLI helpers:"
  echo "  /tmp/gmux-cli ..."
  echo "  $HOME/.local/bin/gmux-dev ..."
  if [[ -n "${CMUX_SHIM_TARGET:-}" ]]; then
    echo "  $CMUX_SHIM_TARGET ..."
  fi
  echo "If your shell still resolves the old gmux, run: rehash"
fi
