...
def _snapshot_path(bundle_id: str) -> Path:
    ...
    return Path.home() / "Library/Application Support/gmux" / f"session-{safe_bundle}.json"
...
def _socket_candidates(app_path: Path, preferred: Path) -> list[Path]:
    ...
    if app_name.startswith(prefix):
        ...
        candidates.append(Path(f"/tmp/gmux-debug-{slug}.sock"))
    deduped: list[Path] = []
    seen: set[str] = set()
...
def main() -> int:
    ...
    socket_path = Path(f"/tmp/gmux-mw-restore-{bundle_suffix}.sock")
