...
if ! grep -Fq 'com.gmuxterm.app.nightly.universal' "$WORKFLOW_FILE"; then
  echo "FAIL: nightly workflow must set a distinct .universal bundle ID"
  exit 1
fi

if ! grep -Fq 'https://github.com/Spectralgo/gmux/releases/download/nightly/appcast-universal.xml' "$WORKFLOW_FILE"; then
