...
_APP_SUPPORT_DIR = os.path.expanduser("~/Library/Application Support/gmux")
_STABLE_SOCKET_PATH = os.path.join(_APP_SUPPORT_DIR, "gmux.sock")
_LEGACY_STABLE_SOCKET_PATH = "/tmp/gmux.sock"
_LAST_SOCKET_PATH_FILES = [
    os.path.join(_APP_SUPPORT_DIR, "last-socket-path"),
    "/tmp/gmux-last-socket-path",
]
_DEFAULT_DEBUG_BUNDLE_ID = "com.gmuxterm.app.debug"
...
def _default_socket_path() -> str:
    ...
    if tag:
        ...
        tagged_candidates = [
            f"/tmp/gmux-debug-{slug}.sock",
            f"/tmp/gmux-{slug}.sock",
        ]
    ...
    candidates = ["/tmp/gmux-debug.sock", _STABLE_SOCKET_PATH, _LEGACY_STABLE_SOCKET_PATH]
    for path in candidates:
        if os.path.exists(path) and _can_connect(path):
            return path

    # Otherwise, fall back to the newest discovered socket if there is one.
    tagged = glob.glob("/tmp/gmux-debug-*.sock")
    tagged.extend(glob.glob(os.path.join(_APP_SUPPORT_DIR, "gmux*.sock")))
