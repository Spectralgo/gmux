...
<p align="center">
  <a href="https://github.com/Spectralgo/gmux/releases/latest/download/gmux-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="下载 Gmux macOS 版" width="180" />
  </a>
</p>
...
<p align="center">
  <img src="./docs/assets/main-first-image.png" alt="Gmux 截图" width="900" />
</p>

<p align="center">
  <a href="https://www.youtube.com/watch?v=i-WxO5YUTOs">▶ 演示视频</a> · <a href="https://cmux.com/blog/zen-of-Gmux">The Zen of Gmux</a>
</p>
...
<td width="40%" valign="middle">
<h3>SSH</h3>
<code>gmux ssh user@remote</code> 为远程机器创建工作区。浏览器窗格通过远程网络路由，因此 localhost 直接可用。将图片拖入远程会话即可通过 scp 上传。
</td>
<td width="60%">
...
<code>gmux claude-teams</code> 一条命令运行 Claude Code 的队友模式。队友以原生分割的形式生成，侧边栏显示元数据和通知。无需 tmux。
</td>
<td width="60%">
...
- **自定义命令** — 在 [`gmux.json`](https://cmux.com/docs/custom-commands) 中定义项目专属操作，通过命令面板启动
...
打开 `.dmg` 文件并将 Gmux 拖动到"应用程序"文件夹。Gmux 通过 Sparkle 自动更新，您只需下载一次。
...
我试过几个编程协调工具，但大多数都是 Electron/Tauri 应用，性能让我不满意。我也更喜欢终端，因为 GUI 协调工具会把你锁定在它们的工作流里。所以我用 Swift/AppKit 构建了 Gmux，作为一个原生 macOS 应用。它使用 libghostty 进行终端渲染，并读取您现有的 Ghostty 配置中的主题、字体和颜色设置。

主要新增的是侧边栏和通知系统。侧边栏有垂直标签页，显示每个工作区的 git 分支、关联 PR 状态/编号、工作目录、监听端口和最新通知文本。通知系统能捕获终端序列（OSC 9/99/777），并提供 CLI（`gmux notify`），您可以将其接入 Claude Code、OpenCode 等代理的钩子。当代理等待时，其窗格会显示蓝色光环，标签页会在侧边栏高亮，这样我就能在多个分割窗格和标签页之间一眼看出哪个需要我。⌘⇧U 可以跳转到最新的未读通知。
...
Gmux 不规定开发者应该如何使用工具。它是一个带有 CLI 的终端和浏览器，其余的由你决定。

Gmux 是原语，而非解决方案。它提供终端、浏览器、通知、工作区、分割、标签页，以及控制这一切的 CLI。Gmux 不强迫你以特定方式使用编程代理。你用这些原语构建什么，完全取决于你自己。
...
有关 Gmux 配置的更多信息，请[查看我们的文档](https://cmux.com/docs/getting-started?utm_source=readme)。
...
[下载 Gmux NIGHTLY](https://github.com/Spectralgo/gmux/releases/download/nightly/gmux-nightly-macos.dmg)

Gmux NIGHTLY 是一个拥有独立 Bundle ID 的单独应用，因此可以与稳定版并行运行。它从最新的 `main` 提交自动构建，并通过独立的 Sparkle 更新源自动更新。

## 会话恢复（当前行为）

重新启动时，Gmux 目前仅恢复应用布局和元数据：
- 窗口/工作区/窗格布局
- 工作目录
- 终端回滚缓冲区（尽力恢复）
- 浏览器 URL 和导航历史

Gmux **不会**恢复终端应用内部的实时进程状态。例如，活动的 Claude Code/tmux/vim 会话在重启后尚无法恢复。
...
- 创建和参与 [GitHub Issues](https://github.com/Spectralgo/gmux/issues) 和[讨论](https://github.com/Spectralgo/gmux/discussions)
- 告诉我们您在用 Gmux 构建什么
...
Gmux 免费、开源，并将一直如此。如果您想支持开发并提前体验即将推出的功能：

**[获取 Founder's Edition](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **功能请求/Bug 修复优先处理**
- **抢先体验：为每个工作区、标签页和面板提供上下文的 Gmux AI**
...
Gmux 以 [GPL-3.0-or-later](LICENSE) 开源。
