...
struct SocketControlSettings {
    ...
    static let legacyStableDefaultSocketPath = "/tmp/gmux.sock"
    static let legacyLastSocketPathFile = "/tmp/gmux-last-socket-path"
    ...
    static func defaultSocketPath(
        ...
        if bundleIdentifier == "com.gmuxterm.app.nightly" {
            return "/tmp/gmux-nightly.sock"
        }
    ...
    static func isDebugLikeBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        ...
        return bundleIdentifier == "com.gmuxterm.app.debug"
            || bundleIdentifier.hasPrefix("com.gmuxterm.app.debug.")
    }

    /// Tagged DEV builds have bundle IDs like `com.gmuxterm.app.debug.<tag>`.
    ...
    static func taggedDebugSocketPath(
        ...
        if bundleId.hasPrefix("\(baseDebugBundleIdentifier).") {
            ...
            if !slug.isEmpty {
                return "/tmp/gmux-debug-\(slug).sock"
            }
        }
        ...
        return "/tmp/gmux-debug-\(tag).sock"
    }

    static func isStagingBundleIdentifier(_ bundleIdentifier: String?) -> Bool {
        guard let bundleIdentifier else { return false }
        return bundleIdentifier == "com.gmuxterm.app.staging"
            || bundleIdentifier.hasPrefix("com.gmuxterm.app.staging.")
    }
