...
enum NamedSessionError: Error, CustomStringConvertible {
    ...
    var description: String {
        switch self {
        case .storageUnavailable:
            return String(localized: "namedSession.error.storageUnavailable", defaultValue: "Session storage directory is unavailable")
        case .invalidName(let reason):
            return String(localized: "namedSession.error.invalidName", defaultValue: "Invalid session name: \(reason)")
        case .notFound(let name):
            return String(localized: "namedSession.error.notFound", defaultValue: "Session '\(name)' not found")
        case .incompatibleVersion(let version, let expected):
            return String(localized: "namedSession.error.incompatibleVersion", defaultValue: "Session version \(version) is incompatible (expected \(expected))")
        case .emptySession:
            return String(localized: "namedSession.error.emptySession", defaultValue: "Session contains no windows")
        case .writeError(let msg):
            return String(localized: "namedSession.error.writeError", defaultValue: "Failed to write session: \(msg)")
        case .readError(let msg):
            return String(localized: "namedSession.error.readError", defaultValue: "Failed to read session: \(msg)")
        }
    }
}
...
enum NamedSessionStore {
    ...
    static func sessionsDirectory(appSupportDirectory: URL? = nil) -> URL? {
        let resolvedAppSupport: URL
        if let appSupportDirectory {
            resolvedAppSupport = appSupportDirectory
