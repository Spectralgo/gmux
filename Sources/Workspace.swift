...
final class Workspace: Identifiable, ObservableObject {
    ...
    func newBeadInspectorSurface(beadId: String, focus: Bool = true) -> BeadInspectorPanel? {
        ...
        let panel = BeadInspectorPanel(
            workspaceId: id,
            beadId: beadId,
            townRootPath: GasTownService.shared.townRoot?.path
        )
    ...
    func newReadyWorkSurface(focus: Bool = true) -> ReadyWorkPanel? {
        ...
        let townRootPath = GasTownService.shared.townRoot?.path
        let panel = ReadyWorkPanel(
            workspaceId: id,
            adapter: BeadsAdapter(townRootPath: townRootPath)
        )
        panels[panel.id] = panel
        panelTitles[panel.id] = panel.displayTitle
    ...
    func newAgentHealthSurface(focus: Bool = true) -> AgentHealthPanel? {
        ...
        let panel = AgentHealthPanel(
            workspaceId: id,
            adapter: AgentHealthAdapter(townRootPath: GasTownService.shared.townRoot?.path)
        )
        panels[panel.id] = panel
        panelTitles[panel.id] = panel.displayTitle
