...
<p align="center">
  <a href="https://github.com/Spectralgo/gmux/releases/latest/download/gmux-macos.dmg">
    <img src="./docs/assets/macos-badge.png" alt="Gmux für macOS herunterladen" width="180" />
  </a>
</p>
...
<td width="40%" valign="middle">
<h3>SSH</h3>
<code>gmux ssh user@remote</code> erstellt einen Arbeitsbereich für eine entfernte Maschine. Browser-Bereiche werden über das entfernte Netzwerk geleitet, sodass localhost einfach funktioniert. Ziehen Sie ein Bild in eine entfernte Sitzung, um es per scp hochzuladen.
</td>
<td width="60%">
...
<code>gmux claude-teams</code> startet den Teammate-Modus von Claude Code mit einem einzigen Befehl. Teammates erscheinen als native Teilungen mit Seitenleisten-Metadaten und Benachrichtigungen. Kein tmux erforderlich.
</td>
<td width="60%">
...
- **Benutzerdefinierte Befehle** — Definieren Sie projektspezifische Aktionen in [`gmux.json`](https://cmux.com/docs/custom-commands), die über die Befehlspalette gestartet werden
...
Öffnen Sie die `.dmg`-Datei und ziehen Sie Gmux in Ihren Programme-Ordner. Gmux aktualisiert sich automatisch über Sparkle, sodass Sie nur einmal herunterladen müssen.
...
Ich habe einige Coding-Orchestratoren ausprobiert, aber die meisten waren Electron/Tauri-Apps und die Performance hat mich gestört. Ich bevorzuge außerdem das Terminal, da GUI-Orchestratoren einen in ihren Workflow einschließen. Also habe ich Gmux als native macOS-App in Swift/AppKit gebaut. Es verwendet libghostty für das Terminal-Rendering und liest Ihre vorhandene Ghostty-Konfiguration für Themes, Schriftarten und Farben.

Die wesentlichen Ergänzungen sind die Seitenleiste und das Benachrichtigungssystem. Die Seitenleiste hat vertikale Tabs, die Git-Branch, verknüpften PR-Status/Nummer, Arbeitsverzeichnis, lauschende Ports und den neuesten Benachrichtigungstext für jeden Arbeitsbereich anzeigen. Das Benachrichtigungssystem erkennt Terminal-Sequenzen (OSC 9/99/777) und bietet eine CLI (`gmux notify`), die Sie in Agent-Hooks für Claude Code, OpenCode usw. einbinden können. Wenn ein Agent wartet, bekommt sein Bereich einen blauen Ring und der Tab leuchtet in der Seitenleiste auf, sodass ich über Teilungen und Tabs hinweg erkennen kann, welcher mich braucht. ⌘⇧U springt zur neuesten ungelesenen Benachrichtigung.
...
Gmux schreibt Entwicklern nicht vor, wie sie ihre Werkzeuge nutzen sollen. Es ist ein Terminal und Browser mit einer CLI, und der Rest liegt bei Ihnen.

Gmux ist ein Grundbaustein, keine fertige Lösung. Es bietet Ihnen ein Terminal, einen Browser, Benachrichtigungen, Arbeitsbereiche, Teilungen, Tabs und eine CLI, um alles zu steuern. Gmux zwingt Sie nicht in eine bestimmte Art, Coding-Agenten zu nutzen. Was Sie mit den Grundbausteinen bauen, ist Ihre Sache.
...
Weitere Informationen zur Konfiguration von Gmux finden Sie in [unserer Dokumentation](https://cmux.com/docs/getting-started?utm_source=readme).
...
[Gmux NIGHTLY herunterladen](https://github.com/Spectralgo/gmux/releases/download/nightly/gmux-nightly-macos.dmg)

Gmux NIGHTLY ist eine separate App mit eigener Bundle-ID, die neben der stabilen Version läuft. Wird automatisch vom neuesten `main`-Commit gebaut und aktualisiert sich über einen eigenen Sparkle-Feed.

## Sitzungswiederherstellung (aktuelles Verhalten)

Beim Neustart stellt Gmux derzeit nur App-Layout und Metadaten wieder her:
- Fenster-/Arbeitsbereich-/Bereichs-Layout
- Arbeitsverzeichnisse
- Terminal-Scrollback (bestmöglich)
- Browser-URL und Navigationsverlauf

Gmux stellt **keine** laufenden Prozesse in Terminal-Apps wieder her. Zum Beispiel werden aktive Claude Code-/tmux-/vim-Sitzungen nach einem Neustart noch nicht fortgesetzt.
...
- Erstellen Sie [GitHub Issues](https://github.com/Spectralgo/gmux/issues) und beteiligen Sie sich an [Diskussionen](https://github.com/Spectralgo/gmux/discussions)
- Lassen Sie uns wissen, was Sie mit Gmux bauen
...
Gmux ist kostenlos, Open Source und wird es immer sein. Wenn Sie die Entwicklung unterstützen und frühen Zugang zu kommenden Funktionen erhalten möchten:

**[Founder's Edition erhalten](https://buy.stripe.com/3cI00j2Ld0it5OU33r5EY0q)**

- **Priorisierte Feature-Requests/Bugfixes**
- **Früher Zugang: Gmux AI, das Ihnen Kontext zu jedem Arbeitsbereich, Tab und Panel gibt**
...
Gmux ist Open Source unter [GPL-3.0-or-later](LICENSE).
